...
function main(yesOrNo) {
  if (yesOrNo) {
    ...
    //不要なレイヤーを削除
    try {
      myDoc.artLayers["RGBA"].remove();
      myDoc.artLayers["UnlitMovieRenderQueue_MotionVectors.RGBA"].remove();
      myDoc.artLayers["UnlitMovieRenderQueue_WorldDepth.RGBA"].remove();
    } catch (e) {
      //何もしない
    }
    ...
    removedDuplicateLayerList.reverse();
...
function moveLayerSetsToLayerSets() {
  ...
  for (i = 0; i < layerNumber; i++) {
    var selectLayer = myDoc.artLayers[0];
    var targetLayerSet = myDoc.layerSets[0];

    selectLayer.move(targetLayerSet, ElementPlacement.INSIDE);
  }
}
